...
def find_common_compartment_item(rucksack_content: str) -> str:
    ...
    # Alternative implementations to learn
    list(filter(lambda item: item in compartment_2, compartment_1))


def filter1(function, array):
    y = []
    for x in array:
        if function(x):
            y += x
...
def find_common_group_badge(rucksack_contents: list[str]) -> str:
    ...
    for item in set(rucksack_contents[0]):
        if item in rucksack_contents[1] and item in rucksack_contents[2]:
            return item
